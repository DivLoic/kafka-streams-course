...
object GameGenerator extends App with Generator {
  ...
  rewards
    .map(reward => new ProducerRecord("rewards-scala", new MachineId(reward.idTerm), reward))
  ...
  while (true) {
    ...
    nextVictory.foreach { victory =>
      ...
      val record: ProducerRecord[MachineId, Victory] =
        new ProducerRecord("victories-scala", new MachineId(victory.terminal), victory)
