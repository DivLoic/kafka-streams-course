...
object LowLevelProcessorApp extends App {
  ...
  builder.table[MachineId, Reward](
    "rewards-scala",
    Materialized.as(Stores.persistentKeyValueStore("reward-store-name"))(machineIdSerde, rewardSerde)
  )(Consumed.`with`(machineIdSerde, rewardSerde))

  val victories: KStream[MachineId, Victory] = builder.stream("victories-scala")(consumed)
  ...
  victories
    ...
    .transform(rewardTransSupplier, "reward-store-name")

    .filter((_, value) => RewardedVictoryError != value)

    .to("rewarded-victories-scala")(Produced.`with`(machineIdSerde, rewardedVictorySerde))
