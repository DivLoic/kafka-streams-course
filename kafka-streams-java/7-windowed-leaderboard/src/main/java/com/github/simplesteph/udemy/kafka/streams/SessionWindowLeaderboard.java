...
public class SessionWindowLeaderboard {
    ...
    private static final Logger logger = LoggerFactory.getLogger(SessionWindowLeaderboard.class);
    ...
    private static final Merger<String, Victories> merger = (aggKey, aggOne, aggTwo) -> new Victories(
            aggOne.getCount() + aggTwo.getCount(),
            aggOne.getLatest().isAfter(aggTwo.getLatest()) ? aggOne.getLatest() : aggTwo.getLatest()
    );
    ...
    public static void main(String[] args) {
        ...
        victories.selectKey((key, value) ->

                        // {"id-terminal": 101, "character": "ryu"}
                        toJsonStringMap(Collections.singletonMap(key, value.getCharacter().getName()))
                )
    ...
    private static String toJsonStringMap(Map<Object, Object> map) {
        try {
