...
object SpecificAvroEventScalaApp extends App {
  ...
  config.put(StreamsConfig.APPLICATION_ID_CONFIG, "specific-avro-scala-app")
  ...
  val usersGlobalTable = builder.globalTable("user-table-avro")

  // we get a stream of user purchases
  val userPurchases = builder.stream("user-purchases-avro")
